...
function customAssign(...objects) {
  ...
  const result1 = Object.assign(objects[0], ...objects);
  ...
  for (const key in result1) {
    if (typeof result1[key] === "number") {
      ...
      result1[key] = result1[key].toString();
    }
  }
...
function customObjectFromEntries(entries) {
  ...
  const valuemap = entries.map(([key, value]) => {
